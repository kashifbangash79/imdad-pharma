...
export default function AgentPayment({ onSubmit }) {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/agent-payments');
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingIndex !== null) {
      ...
      try {
        const response = await axios.put(`http://localhost:3000/api/agent-payments/update/${history[editingIndex]._id}`, shipmentDetails);
      ...
      // Add new entry
      try {
        const response = await axios.post('http://localhost:3000/api/agent-payments/save', shipmentDetails);
